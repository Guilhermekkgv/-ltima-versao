...
function Linux.Create(config)
    ...
    local TabHolder = Linux.Instance("ScrollingFrame", {
        Parent = TabsBar,
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 0, 0, 0), -- Garantir que o TabHolder comece no topo do TabsBar
    ...
    local TabIndicator = Linux.Instance("Frame", {
        Parent = TabHolder,
        BackgroundColor3 = Linux.Theme.Accent,
        Size = UDim2.new(0, 4, 0, 20),
        Position = UDim2.new(0, 0, 0, 0), -- Posição inicial será ajustada abaixo
        ZIndex = 3
    })
    ...
    function LinuxLib.Tab(config)
        ...
        TabBtn.MouseButton1Click:Connect(function()
            ...
            -- Novo cálculo para a posição Y da barrinha
            local tabHeight = 28
            local tabSpacing = 3
            local indicatorHeight = 20
            local paddingTop = 5
            local centerOffset = (tabHeight - indicatorHeight) / 2
            local targetY = paddingTop + (tabIndex - 1) * (tabHeight + tabSpacing) + centerOffset

            -- Log para depuração
            print("TabIndex:", tabIndex, "TargetY:", targetY)

            local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local success, err = pcall(function()
                TweenService:Create(TabIndicator, tweenInfo, {Position = UDim2.new(0, 0, 0, targetY)}):Play()
            end)
            if not success then
                warn("Erro na animação do TabIndicator:", err)
            end
        end)
        ...
        if not CurrentTab then
            ...
            -- Novo cálculo para a posição inicial da barrinha
            local tabHeight = 28
            local tabSpacing = 3
            local indicatorHeight = 20
            local paddingTop = 5
            local centerOffset = (tabHeight - indicatorHeight) / 2
            local initialY = paddingTop + (tabIndex - 1) * (tabHeight + tabSpacing) + centerOffset
            print("InitialY:", initialY)
            TabIndicator.Position = UDim2.new(0, 0, 0, initialY)
        end
