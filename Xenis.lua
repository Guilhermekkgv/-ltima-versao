...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        TabBtn.MouseButton1Click:Connect(function()
            ...
            TabBtn:GetChildren()[2].TextColor3 = Color3.fromRGB(255, 255, 255)
            if TabIcon then
                TabIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
            end
            CurrentTab = tabIndex

            local targetY = 5 + (tabIndex - 1) * (28 + 3)
            local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            TweenService:Create(TabIndicator, tweenInfo, {Position = UDim2.new(0, 0, 0, targetY), Size = UDim2.new(0, 4, 0, 28)}):Play()
        end)

        Tabs[tabIndex] = {
            Name = config.Name,
            Button = TabBtn,
            Text = TabBtn:GetChildren()[2],
            Icon = TabIcon,
            Content = TabContent
        }
        ...
        if not CurrentTab then
            ...
            TabIndicator.Size = UDim2.new(0, 4, 0, 28)
        end
        ...
        function TabElements.Dropdown(config)
            ...
            local function UpdateDropSize()
                ...
                task.wait(0.2)
                TabContent.CanvasSize = UDim2.new(0, 0, 0, ElementContainer:GetChildren()[1].AbsoluteContentSize.Y)
            end
